...
export default class FileDropWithPreview extends Component {
  ...
  render() {
    const { isUserDraggingFile } = this.state;
    const {
      image,
      input,
      loading,
    ...
    return (
      <div>
        <FileDrop onDrop={this.dropHandler} className="FileDrop">
          <div className={`${isUserDraggingFile ? `DroppableAreaImageHolder ${circular ? 'DroppableAreaImageHolderCircular' : ''}` : ''}`}>
            <Dimmer active={loading} inverted>
              <Loader inverted/>
            </Dimmer>
            <div className="DroppableArea" ref="dropArea">
              <Image onClick={this.clickHandler} {...image} inline />
            </div>
          </div>
          <input ref='imageInput' type='file' className='hide' accept='image/*' {...input} />
        </FileDrop>
      </div>
    );
  }
